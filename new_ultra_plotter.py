...
def dupesinfo(data, channels):
    ...
    for chan in range(channels):
        ...
        for run in data:
            ...
            agents.fillna("nan",inplace=True)

            dupes_run = ((agents[agents.columns] != -1) & (agents[agents.columns] != 'nan')).sum(1)
...
def memoryspread(data, nagents, channels):

    spread = pd.DataFrame()
    
    for run in data:

        memoryused = pd.DataFrame()

        for n in range(nagents):

            chans = run.filter(items=['c{}_{}'.format(n, x) for x in range(channels)])

            memoryused = pd.concat([memoryused,np.logical_and(chans != -1, chans.notna()).sum(axis=1).mask((~chans.notna()).any(axis=1))], axis=1)

        spread = pd.concat([spread, memoryused], axis=1)
...
# alpha=0.3
# can be used to lighten from the background filler line

def main():
    filenames = ['Dynamic_Migration/Static_Move_non',
                'Dynamic Heursitic/Cicrle_Move_non',
                'Dynamic Heursitic/Circle_Move_con',
                'Dynamic Heursitic/Static_Move_con',
                'Dynamic Heursitic/Static_Move_non',
                'Static Heurisitic/Circle_movement_concurrent',
                'Static Heurisitic/Circle_movement_non',
                'Static Heurisitic/Static_Movement_concurrent',
                'Static Heurisitic/Static_Movement_non']
    repeats = 5
    ...
    for filename in filenames:
        ...
        for i in range(repeats):
            df = pd.read_csv(filename + '/SimpleSuicideReplication_{}.csv'.format(i+1), header=None, sep='\n')
        ...
        print("Complete stability")
        spread = memoryspread(runs_data, amount_agents, total_datas)
        print('Complete spread')
        ...
        for i in range(1 + len(ratio_mean)//3):
            for j in range(3):
                if i*3+j < len(ratio_mean):
                    fig_dists.append(fig3.add_subplot(gs[j*10:j*10+10, 4+i])) # Change to 1 with large data
                    #fig_dists.append(fig3.add_subplot(gs[0:3, 4:7]))
        ...
        line = f3_ax1.fill_between( iterations, gaussian_filter1d((np.array(t_mean_2) + np.array(t_mean_1)), sigma=50), gaussian_filter1d((np.array(t_mean_2) - np.array(t_mean_1)), sigma=50),alpha=0.5)
        line.set_color('green')
        ...
        line = f3_ax2.fill_between( iterations, gaussian_filter1d((np.array(t2) + np.array(t1)), sigma=50), gaussian_filter1d((np.array(t2) - np.array(t1)), sigma=50),alpha=0.5)
        line.set_color('green')

        line, = f3_ax2.plot( iterations, t2 )
        ...
        line = f3_ax3.fill_between( iterations, gaussian_filter1d(np.array(t_mean_3), sigma=50), gaussian_filter1d(np.array(t_mean_4), sigma=50), alpha=0.5)
        line.set_color('orange')
        ...
        #     line.set_color('green')
        line = f3_ax3.fill_between( iterations, gaussian_filter1d((np.array(t_mean_2) + np.array(t_mean_1)), sigma=50), gaussian_filter1d((np.array(t_mean_2) - np.array(t_mean_1)), sigma=50), alpha=0.5 )
        line.set_color('green')
        ...
        stab_mean = np.mean(stab_data/5, axis=0)
        stab_std = np.std(stab_data/5, axis=0)

        line = f3_ax4.fill_between( iterations[1:], gaussian_filter1d((stab_mean + stab_std), sigma=50), gaussian_filter1d((stab_mean - stab_std), sigma=50), alpha=0.5)
        line.set_color('green')
        f3_ax4.plot(iterations[1:], gaussian_filter1d(stab_data, sigma=50).T/5)
        ...
        plt.savefig('{}/{}.png'.format(filename.split('/')[0], filename.split('/')[-1]), bbox_inches='tight')
